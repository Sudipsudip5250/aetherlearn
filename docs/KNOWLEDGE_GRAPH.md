...
Repo: [Sudipsudip5250/aetherlearn](https://github.com/Sudipsudip5250/aetherlearn) · License MIT · product name **AetherLearn**.

## Pointer (update every session)

| Field | Value |
|---|---|
| Last graph update | 2026-09-14 |
| `main` at last update | Squashed public history + motion polish; author `sudipsudip5250` |
| Last merged product PR | PRs #1–#6 were merged before history rewrite; `main` is now a short linear history |
| Tester APK | [testing](https://github.com/Sudipsudip5250/aetherlearn/releases/tag/testing) · `AetherLearn-debug.apk` · debug-signed, not production |
| Next agent work | Human phone testing. Do not add Stage 6. Rebuild APK only via workflow_dispatch if icons/motion need a new tester build. |
| Do not do | Stage 6 tracks, accounts, analytics, cloud sync, signing keys, claiming device/TalkBack pass |
| CI budget | Quality `validate` on push/PR (~1 min). Android assemble only via `workflow_dispatch` or `Publish debug APK`. |
