...
- Release: https://github.com/Sudipsudip5250/aetherlearn/releases/tag/testing
- File: `AetherLearn-debug.apk`
- Checksum sidecar: `AetherLearn-debug.apk.SHA256SUMS`
- Direct APK: https://github.com/Sudipsudip5250/aetherlearn/releases/download/testing/AetherLearn-debug.apk
- Also on that release: `aetherlearn-visual-foundations-1.1.0.zip` and `aetherlearn-core-pack-1.0.0.zip` plus checksums
...
The committed sample artifact is [`sample-pack/aetherlearn-core-pack-1.0.0.zip`](sample-pack/aetherlearn-core-pack-1.0.0.zip), with checksum sidecar [`sample-pack/aetherlearn-core-pack-1.0.0.zip.SHA256SUMS`](sample-pack/aetherlearn-core-pack-1.0.0.zip.SHA256SUMS). It contains the 37 current bundled lessons—the original 20-module MVP baseline plus the three approved Stage 1 lessons, four Stage 2 drafts, four Stage 3 defensive-security drafts, four Stage 4 Web/data drafts, and two Stage 5 historical-security/career drafts—along with a root `manifest.json` and `modules/<filename>` entries.
...
The separate local fixture [`sample-pack/aetherlearn-visual-foundations-1.1.0.zip`](sample-pack/aetherlearn-visual-foundations-1.1.0.zip) contains exactly 38 files: `manifest.json` and one static SVG diagram plus trace-practice metadata for each of the 37 current lessons. Rebuild it deterministically with `python3 scripts/build_visual_pack.py --source-dir media/visuals/visual-foundations --output docs/sample-pack/aetherlearn-visual-foundations-1.1.0.zip`; the builder validates the source and generated ZIP and emits the adjacent `.SHA256SUMS` sidecar. Validate it with `python3 scripts/validate_visual_pack.py docs/sample-pack/aetherlearn-visual-foundations-1.1.0.zip` and verify `sha256sum -c docs/sample-pack/aetherlearn-visual-foundations-1.1.0.zip.SHA256SUMS`. It is not a replacement for the `pack_id: core` lesson sample, is not accepted by the Android HTTPS lesson downloader, and is not a signed or public release pack.

On Android, use **Settings → Storage & content packs → Visual Foundations → Install local pack** when the fixture is bundled. Confirm the installed pack does not add lessons, then open representative lessons from each strand—such as DL-01, PY-01, AL-03, DEV-03, SEC-02, WEB-01, and SEC-06—to inspect the optional diagrams, text equivalents, attribution, and observe-and-trace activity. Delete it and confirm the core pack and learning records remain. On Web, serve `web/` locally, use **About → Optional visual pack**, and follow the visual steps in [`DEVICE_TEST_CHECKLIST.md`](DEVICE_TEST_CHECKLIST.md). The Web flow stores only same-origin Cache Storage entries; the Android flow uses app-private files. The fixture has unsigned-development metadata and no remote visual download, publisher signing, audio/video, streaming, Media3, OBB, or PAD path.
...
```text
https://raw.githubusercontent.com/Sudipsudip5250/aetherlearn/main/docs/sample-pack/aetherlearn-core-pack-1.0.0.zip
```
...
The current repository is suitable for controlled human testing. The debug APK is the normal unsigned-by-project test artifact; the release APK is explicitly unsigned and must be signed before it can be installed or distributed. The repository is not production-signed or publicly approved. The release owner must still obtain human approval for all 37 current lessons, including the original 20-module MVP baseline, the three draft Stage 1 lessons, the four draft Stage 2 lessons, the four draft Stage 3 lessons, the four draft Stage 4 lessons, and the two draft Stage 5 lessons, complete representative Android/emulator and Termux tests, complete Android-browser and assistive-technology tests, complete the explicit Stage 3 safety review, Stage 4 Web/data accessibility/privacy review, and Stage 5 historical/cultural/source/career-framing review, establish authorized signing and key rotation, verify signed metadata, and approve the distribution and pack-host process.
