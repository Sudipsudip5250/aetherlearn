...
M6 has started with a static web shell under `web/`. The client reuses the five canonical Markdown files through a small manifest and dependency-free browser parser, renders a responsive lesson catalog and full reader route, preserves the Android-only Termux boundary, and includes privacy/accessibility foundations. `scripts/check_web_content.py` verifies that the web payload mirrors the canonical source exactly. JavaScript syntax, JSON, repository links, content pack, Python tests, and Git diff checks pass. Service-worker caching, explicit IndexedDB pack storage, browser progress/notes/bookmarks, local search, practice flow, and offline network-disabled testing remain the next M6 slices.
