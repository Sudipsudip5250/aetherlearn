...
M5 is implemented for the feasible code and documentation scope. Two S1 local-only wrappers are selected: `py-02-local-expressions` from PY-02 and `dev-01-safe-navigation` from DEV-01. `docs/TERMUX_WRAPPERS.md` defines contract version 1, fixed executable paths, fixed argument arrays, dedicated working directories, prerequisites, expected effects, offline fallbacks, learner-confirmed completion, and explicit non-goals. `TermuxWrappers.kt` contains the immutable registry and strict request validation. `TermuxBridge.kt` detects the `com.termux` package and sends only the documented RUN_COMMAND action, explicit service component, command path, fixed argument array, fixed working directory, foreground flag, and session action. The lesson reader shows package status, setup guidance, confirmation details, a fallback, and learner-confirmed completion backed by schema-versioned local storage. M5 unit tests cover unknown IDs, altered arguments, altered executable/path, and contract-version tampering. Physical device/emulator tests for Termux variants, permission denial, service configuration, and successful handoff remain evidence-limited because no device/emulator is available here.

M6 is implemented for the secondary static web/PWA fallback. The client reuses the five canonical Markdown files through a verified manifest and dependency-free browser parser, renders the catalog, full reader, offline practice, local search, progress, notes, bookmarks, and short-answer knowledge checks, and preserves the Android-only Termux boundary. The service worker caches the app shell, while IndexedDB stores a staged/active content pack and versioned browser-local learning state. Browser verification with Chromium and the local server stopped confirmed five-lesson catalog/reader access, practice, search, progress, quiz feedback, note/bookmark/completion persistence, Termux messaging, active service-worker control, and five IndexedDB entries. `scripts/check_web_content.py`, JavaScript syntax checks, JSON, repository links, content-pack validation, Python tests, Android checks, and Git diff checks remain release checks. Android-browser runtime evidence is still open because no Android device/browser is attached here.
...
| D-025 | 2026-08-25 | Mark the M6 desktop-browser checkpoint complete while leaving Android-browser smoke testing open | Chromium verification with the server stopped proved the cached shell, five-lesson IndexedDB pack, local learning state, practice, and search flow; no Android browser/device is available for a second runtime matrix | An attached Android browser/device completes the remaining evidence gate |
