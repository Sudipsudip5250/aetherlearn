...
**Status:** planning only; no current MVP module or client payload changes.

AetherLearn’s current curriculum is intentionally frozen at 20 modules. It provides a coherent foundation in digital literacy, Python, basic algorithms, and developer practice. It does not attempt to cover every programming language, operating system, organization, security topic, or career path. This document records what could come next so future contributors do not expand the product ad hoc or turn historical and security material into unsafe operational instruction.
...
The gap analysis is a planning assessment, not evidence that the MVP is incomplete. The current release should not be expanded until its human pedagogical and safety review and device gates are complete.
...
A release candidate should keep new content outside `content/curriculum.yml` until the project explicitly approves a curriculum change. Once a future module is approved, it must use the existing validator, Android/Web byte-parity process, and human pedagogical/safety review. This preserves the current frozen 20-module MVP and prevents a roadmap from silently becoming product scope.
...
The immediate project priority remains release verification of the existing 20 modules—not adding this roadmap to the application. The roadmap is complete when contributors can choose a clearly bounded topic, identify authoritative sources, explain the safety boundary, and obtain explicit scope approval before authoring.
