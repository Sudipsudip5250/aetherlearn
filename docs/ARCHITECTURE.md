...
The current `visual-foundations` fixture remains local-only and bounded, but its 1.1.0 manifest now associates one optional static SVG diagram and one observe-and-trace activity with each of the 37 approved lesson IDs. Every asset carries raw installed/source-payload bytes, SHA-256, alt text, caption, complete text equivalent, reduced-motion wording, license, attribution, author, locale, and bounded practical metadata. The generated ZIP’s actual transport size and SHA-256 sidecar are recorded separately because a source manifest cannot know a future archive’s compression result without a second generated manifest. The pack remains supplementary: no lesson frontmatter, completion rule, quiz state, notes, bookmarks, or core content is dependent on it.
...
No audio/video, streaming, arbitrary user media, remote visual catalog, visual HTTPS downloader, signing key, public host, OBB, Media3 dependency, or PAD dependency is part of this slice. The visual pack contains no executable or arbitrary HTML/JavaScript content; each practical aid is a static prompt plus learner-readable steps and self-check. PAD remains a future Play-only optimization and not the canonical delivery path for open-source, sideloaded, or user-selected packs.
