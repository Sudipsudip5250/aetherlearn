...
Core content is bundled or downloaded during first-run setup. Optional media packs are independently downloadable and deletable. A new app version must remain able to open the last compatible core pack. The PWA uses a versioned service-worker cache for its application shell and a separate IndexedDB content-pack store; service-worker caching alone is not treated as proof that the curriculum is offline-available.
