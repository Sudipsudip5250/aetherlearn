...
M0 through M6 are implemented for the current feasible scope, and the next content batch is now authored and synchronized. The native Kotlin/Jetpack Compose Android app discovers eleven bundled lessons, tracks learning state and quiz attempts, stores private notes/bookmarks, searches locally, lists practice exercises, exports learning data as Markdown/JSON through a user-controlled file picker, manages a validated pre-bundled optional-pack example, and provides a strict versioned Termux allowlist, package detection, explicit confirmation, guarded RUN_COMMAND handoff, fallback guidance, and learner-confirmed completion for four local-only exercises. The `web/` fallback reuses the eleven canonical Markdown lessons, caches its shell with a service worker, stores the explicit eleven-lesson pack and browser-local learning state in IndexedDB, and provides offline reading, practice, search, progress, notes, bookmarks, and knowledge checks. Android device/emulator and Android-browser smoke tests remain unavailable runtime evidence gates. See [`android/README.md`](android/README.md), [`web/README.md`](web/README.md), and [`docs/TERMUX_WRAPPERS.md`](docs/TERMUX_WRAPPERS.md).
