...
M0 through M4 are implemented for the current offline scope, the feasible M5 code slice is implemented, and the initial M6 web shell is now available. The native Kotlin/Jetpack Compose Android app reads the five bundled lessons, tracks learning state and quiz attempts, stores private notes/bookmarks, searches locally, lists practice exercises, exports learning data as Markdown/JSON through a user-controlled file picker, manages a validated pre-bundled optional-pack example, and provides a strict versioned Termux allowlist, package detection, explicit confirmation, guarded RUN_COMMAND handoff, fallback guidance, and learner-confirmed completion for two local-only exercises. The new `web/` client reuses the canonical Markdown lessons and renders a responsive catalog and reader. Service-worker/IndexedDB caching and browser learning state are the next M6 slice. Device/emulator smoke testing remains the unavailable native runtime evidence gate. See [`android/README.md`](android/README.md), [`web/README.md`](web/README.md), and [`docs/TERMUX_WRAPPERS.md`](docs/TERMUX_WRAPPERS.md).
